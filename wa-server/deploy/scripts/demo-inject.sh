#!/usr/bin/env bash
# Demo 数据注入脚本 (wa-server)
# 用法: ./wa-server/deploy/scripts/demo-inject.sh [command]
#
# Commands:
#   pair    <phone>       注入配对码任务 (默认使用测试号 8618666206882)
#   pair-qr                注入 QR 配对任务
...
#
# 示例:
#   ./wa-server/deploy/scripts/demo-inject.sh pair 8618666206882
#   ./wa-server/deploy/scripts/demo-inject.sh events
#   ./wa-server/deploy/scripts/demo-inject.sh seed
...
示例:
  ./wa-server/deploy/scripts/demo-inject.sh pair 8618666206882
  ./wa-server/deploy/scripts/demo-inject.sh events
  ./wa-server/deploy/scripts/demo-inject.sh send 8618666206882@s.whatsapp.net "你好"
USAGE
        ;;
esac
