...
//!
//! Lives in `wacore` (not the top-level `whatsapp-rust` crate) so that storage
//! backend crates like `whatsapp-rust-postgres-storage` can implement it
//! without a circular dependency on the main crate.
...
/// device row should be created on first sight. The factory itself must be
/// cheap to clone (wrap internals in `Arc`).
#[async_trait]
