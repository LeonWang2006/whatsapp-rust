...
--
-- Mirrors the SQLite schema (storages/sqlite-storage/migrations) using
-- PG-native types:
--   BLOB                        -> BYTEA
...
--   BIGINT                       -> BIGINT
--   BOOLEAN                      -> BOOLEAN
-- All account-specific tables carry `device_id` as part of a composite
-- primary key, enabling many WhatsApp accounts to share one database. Rows
-- reference `device(id)` with ON DELETE CASCADE so deleting a device row
-- removes all of its account data.
...
CREATE INDEX idx_group_metadata_device ON group_metadata (device_id);
...
-- Inbound durability hook: a decrypted message is buffered here (keyed by its
-- stanza id) before the Signal ratchet is flushed, so a crash before the hook
-- acks replays the message on redelivery instead of dropping it.
CREATE TABLE pending_inbound_messages (
    ...
    device_id   INTEGER NOT NULL REFERENCES device(id) ON DELETE CASCADE,
    inserted_at BIGINT  NOT NULL DEFAULT (EXTRACT(EPOCH FROM clock_timestamp())::BIGINT),
...
-- Per-outbound-message secret for addon-style decryption (poll votes,
-- reactions, msmsg bot responses, edits). `expires_at = 0` means "never".
...
CREATE INDEX idx_msg_secrets_expiry ON msg_secrets (expires_at, device_id);
